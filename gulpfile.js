...
/**
 * Gulp namespace, ensures that ./apps/pl/pattern-lab/config.yml &
 * ./apps/drupal/themename.info.yml are updated with all pattern namespaces for
...
gulp.task('compile:twig-namespaces', () => gulp
    ...
    .pipe(twigNamespaces({
        ...
        outputs: [
            {
                // Note: PL will NOT compile unless the namespaces are
                // explicitly declared
            ...
            {
                // The component-libraries module wants to know about our
                // namespaces
                configFile: path.join(PATH_DRUPAL, 'starter_theme.info.yml'),
                atKey: 'component-libraries',
                pathRelativeToDir: path.join(PATH_DRUPAL, ''),
            },
        ],
        // What are the top-level namespace paths, and which sub paths should
        // we ignore?
        sets: {
            protons: {
...
/**
 * When PL is done compiling do stuff here to notify anything that needs to
 * know. Currently it just writes a file to the root of dist/ so that Webpack
 * can trigger a reload.
 */
gulp.task('compile:pl:notify', (cb) => {
    // Write to dist/ root a file named CHANGED.txt, casts Date to text, calls
    // callback
    fs.writeFile(path.resolve(__dirname, PATH_DIST, 'CHANGED.txt'), +new Date(), cb);
});

/**
 * Generate data json PL uses to determine which mode: 'development' or
